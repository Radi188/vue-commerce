...
import { updateProfiles } from '@/firebase';
import { getAuth } from '@firebase/auth';

const auth = getAuth();
let user = auth.currentUser;

const profile = reactive({name: '' , email: '', phone: '', address: '', postcode: '' })
const account = reactive({name: '' , email: '', photoUrl: '', emialVerified: '', password: '', confirmPassword: '' })
const uploadImage = () => {
  console.log('/')
}
const resetPassword = () => {
...
const updateProfile = () => {
  updateProfiles(user.uid, profile.name , profile.phone, profile.address, profile.postcode)
  profile.name = ''
  profile.phone = ''
  profile.address = ''
  profile.postcode = ''
